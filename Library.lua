    ...
    -- optimized more and modified by fiji and eternal
]]
-- // Variables
...
local Clamp, Round, Floor, Acos, Cos, Abs, Sin, Pi = math.clamp, math.round, math.floor, math.acos, math.cos, math.abs, math.sin, math.pi
local Find, Remove, Unpack, Insert = table.find, table.remove, table.unpack, table.insert
...
local library = {
    ...
    folders = {
        main = "Sanity",
        assets = "Sanity/Assets",
        configs = "Sanity/Presets"
    },
    shared = {
        initialized = false,
        fps = 0,
...
local sections = {}
local theme = {
    accent = NewRGB(55, 175, 225),
    lightcontrast = NewRGB(30, 30, 30),
    darkcontrast = NewRGB(20, 20, 20),
    outline = NewRGB(0, 0, 0),
    inline = NewRGB(50, 50, 50),
    textcolor = NewRGB(255, 255, 255),
    textdark = NewRGB(175, 175, 175),
    textborder = NewRGB(0, 0, 0),
    cursoroutline = NewRGB(10, 10, 10),
    font = 2,
    textsize = 13
}
library.theme = theme
...
do
	...
	function utility:Create(instanceType, instanceOffset, instanceProperties, instanceParent)
        ...
        local instanceOffset = instanceOffset or {NewVector2(0,0)}
        local instanceProperties = instanceProperties or {}
        local instanceHidden = false
        local instance = nil
        --
		if instanceType == "Frame" or instanceType == "frame" then
            local frame = NewDrawing("Square")
            frame.Visible = true
            frame.Filled = true
            frame.Thickness = 0
            frame.Color = NewRGB(255,255,255)
            frame.Size = NewVector2(100,100)
            frame.Position = NewVector2(0,0)
            frame.ZIndex = 1000
        ...
        elseif instanceType == "Image" or instanceType == "image" then
            local image = NewDrawing("Image")
            image.Size = NewVector2(12,19)
            image.Position = NewVector2(0,0)
            image.Visible = true
            image.ZIndex = 1000
        ...
        elseif instanceType == "Quad" or instanceType == "quad" then
            local quad = NewDrawing("Quad")
            quad.Visible = false
            quad.Color = NewRGB(255, 255, 255)
            quad.Thickness = 1.5
            quad.Transparency = 1
            quad.ZIndex = 1000
            quad.Filled = false
            ...
            for i, v in pairs(instanceProperties) do
                ...
                --[[if typeof(v) == "Color3" then
                    ...
                    if found_theme then
                        ...
                        Insert(themes[found_theme][i], instance)
                    end
                end]]
            end
            --
    ...
    function utility:Remove(instance, hidden)
        ...
        for i,v in pairs(hidden and library.hidden or library.drawings) do
            if v[1] == instance then
                v[1] = nil
                v[2] = nil
                Remove(hidden and library.hidden or library.drawings, i)
                break
            end
        end
    ...
    function utility:GetTextBounds(text, textSize, font)
        local textbounds = NewVector2(0, 0)
        --
        local textlabel = utility:Create("TextLabel", {NewVector2(0, 0)}, {
            Text = text,
            Size = textSize,
            Font = font,
            Hidden = true
...
do
    ...
    function library:New(info)
        ...
        local size = info.size or info.Size or NewVector2(504,604)
        local accent = info.accent or info.Accent or info.color or info.Color or theme.accent
        local pageamount = info.Pages
        --
        theme.accent = accent
        --
        local window = {pages = {}, isVisible = false, pageammount = pageamount,uibind = Enum.KeyCode.Z, currentPage = nil, fading = false, dragging = false, drag = NewVector2(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil}}
        --
        local main_frame = utility:Create("Frame", {NewVector2(0,0)}, {
        ...
        --
        local frame_inline = utility:Create("Frame", {NewVector2(1,1), main_frame}, {
        ...
        --
        local inner_frame = utility:Create("Frame", {NewVector2(1,1), frame_inline}, {
        ...
        --
        local title = utility:Create("TextLabel", {NewVector2(4,2), inner_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
        ...
        --
        local inner_frame_inline = utility:Create("Frame", {NewVector2(4,18), inner_frame}, {
        ...
        --
        local inner_frame_inline2 = utility:Create("Frame", {NewVector2(1,1), inner_frame_inline}, {
        ...
        --
        local back_frame = utility:Create("Frame", {NewVector2(1,1), inner_frame_inline2}, {
        ...
        --
        local tab_frame_inline = utility:Create("Frame", {NewVector2(4,24), back_frame}, {
        ...
        --
        local tab_frame_inline2 = utility:Create("Frame", {NewVector2(1,1), tab_frame_inline}, {
        ...
        --
        local tab_frame = utility:Create("Frame", {NewVector2(1,1), tab_frame_inline2}, {
        ...
        do
            window.VisualPreview = {
                Size = {X = 5, Y = 0},
                Color1 = NewRGB(0, 255, 0),
                Color2 = NewRGB(255, 0, 0),
                HealthBarFade = 0,
                Fading = false,
                State = false,
                Visible = true,
            ...
            --
            local esppreview_frame = utility:Create("Frame", {NewVector2(main_frame.Size.X + 5,0), main_frame}, {
            ...
            --
            local esppreview_inline = utility:Create("Frame", {NewVector2(1,1), esppreview_frame}, {
            ...
            --
            local esppreview_inner = utility:Create("Frame", {NewVector2(1,1), esppreview_inline}, {
            ...
            --
            local esppreview_title = utility:Create("TextLabel", {NewVector2(4,2), esppreview_inner}, {
                Text = "ESP Preview",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            --
            local esppreview_visiblebutton = utility:Create("TextLabel", {NewVector2(esppreview_inner.Size.X - (5 + 7),2), esppreview_inner}, {
                Text = "O",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            --
            local esppreview_inner_inline = utility:Create("Frame", {NewVector2(4,18), esppreview_inner}, {
            ...
            --
            local esppreview_inner_outline = utility:Create("Frame", {NewVector2(1,1), esppreview_inner_inline}, {
            ...
            --
            local esppreview_inner_frame = utility:Create("Frame", {NewVector2(1,1), esppreview_inner_outline}, {
            ...
            --
            local esppreview_frame_previewbox = utility:Create("Frame", {NewVector2(10,10), esppreview_inner_frame}, {
            ...
            function window.VisualPreview:UpdateHealthBar()
                ...
                local Smoothened = (Acos(Cos(window.VisualPreview.HealthBarFade * Pi)) / Pi)
                ...
                --
                healthvalue.Text = Round(Smoothened * 100).. 'HP'
                ...
                utility:UpdateOffset(healthbar, {NewVector2(1, healthbaroutline.Size.Y - Size - 1), healthbaroutline})
            end
            --
            function window.VisualPreview:UpdateHealthValue(Size)
                local New = NewVector2(healthbar.Position.X + (5 - Size), Clamp(healthbar.Position.Y + 5, 0, (healthbar.Position.Y) + (healthbar.Size.Y) - 18))
                --
                healthvalue.Position = New
                utility:UpdateOffset(healthvalue, {NewVector2(5 - Size, New.Y - healthbar.Position.Y), healthbar})
            end
            --
            function window.VisualPreview:ValidateSize(Side, Size)
                if not (window.VisualPreview.Size[Side] == Size) then
                    ...
                    --
                    utility:UpdateOffset(esppreview_visiblebutton, {NewVector2(esppreview_inner.Size.X - (5 + 7),2), esppreview_inner})
                    utility:UpdateOffset(esppreview_frame_previewbox, {NewVector2(10,10), esppreview_inner_frame})
                    utility:UpdateOffset(boxoutline, {NewVector2(esppreview_frame_previewbox.Size.X - BoxSize.X - 1, 20), esppreview_frame_previewbox})
                    --
                    window:Move(main_frame.Position + NewVector2(0, 0))
                end
            end
            --
            ...
            --
            do -- Preview Stuff
                local preview_boxoutline = utility:Create("Frame", {NewVector2(esppreview_frame_previewbox.Size.X - BoxSize.X - 1, 20), esppreview_frame_previewbox}, {
                    Size = BoxSize,
                ...
                --
                local preview_box = utility:Create("Frame", {NewVector2(0, 0), preview_boxoutline}, {
                ...
                --
                local preview_heatlhbaroutline = utility:Create("Frame", {NewVector2(-6, -1), preview_boxoutline}, {
                ...
                --
                local preview_heatlhbar = utility:Create("Frame", {NewVector2(1, 1), preview_heatlhbaroutline}, {
                ...
                --
                local preview_title = utility:Create("TextLabel", {NewVector2(preview_box.Size.X / 2, -20), preview_box}, {
                    Text = "Username",
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = theme.textcolor,
                ...
                --
                local preview_distance = utility:Create("TextLabel", {NewVector2(preview_box.Size.X / 2, preview_box.Size.Y + 5), preview_box}, {
                    Text = "25m",
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = theme.textcolor,
                ...
                --
                local preview_tool = utility:Create("TextLabel", {NewVector2(preview_box.Size.X / 2, preview_box.Size.Y + 20), preview_box}, {
                    Text = "Weapon",
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = theme.textcolor,
                ...
                --
                local preview_character = utility:Create("Frame", {NewVector2(46/2, 40/2), preview_box}, {
                ...
                do -- Chams
                    for Index = 1, 2 do
                        ...
                        local color = Index == 1 and NewRGB(93, 62, 152) or NewRGB(255, 255, 255)
                        --
                        local extrasize = Index == 1 and 4 or 0
                        local extraoffset = Index == 1 and -2 or 0
                        --
                        local preview_character_head = utility:Create("Frame", {NewVector2((preview_character.Size.X * 0.35) + (extraoffset), extraoffset), preview_character}, {
                        ...
                        --
                        local preview_character_torso = utility:Create("Frame", {NewVector2((preview_character.Size.X * 0.25) + (extraoffset), (preview_character.Size.Y * 0.2) + (extraoffset)), preview_character}, {
                        ...
                        --
                        local preview_character_leftarm = utility:Create("Frame", {NewVector2(extraoffset, (preview_character.Size.Y * 0.2) + (extraoffset)), preview_character}, {
                        ...
                        --
                        local preview_character_rightarm = utility:Create("Frame", {NewVector2((preview_character.Size.X * 0.75) + (extraoffset + extrasize), (preview_character.Size.Y * 0.2) + (extraoffset)), preview_character}, {
                            Size = utility:Size(0.25, 0, 0.4, extrasize, preview_character),
                        ...
                        --
                        local preview_character_leftleg = utility:Create("Frame", {NewVector2((preview_character.Size.X * 0.25) + (extraoffset), (preview_character.Size.Y * 0.6) + (extraoffset + extrasize)), preview_character}, {
                        ...
                        --
                        local preview_character_rightleg = utility:Create("Frame", {NewVector2((preview_character.Size.X * 0.5) + (extraoffset + (extrasize / 2)), (preview_character.Size.Y * 0.6) + (extraoffset + extrasize)), preview_character}, {
                            Size = utility:Size(0.25, extrasize / 2, 0.4, 0, preview_character),
                            Position = utility:Position(0.25, extraoffset, 0.6, extraoffset + extrasize, preview_character),
                            Color = color,
                            Transparency = transparency
                ...
                do -- Skeleton
                    for Index = 1, 2 do
                        local skeletonsize = NewVector2(Index == 1 and 3 or 1, Index == 1 and -10 or -12)
                        local skeletonoffset = NewVector2(Index == 1 and -1 or 0, Index == 1 and 5 or 6)
                        local transparency = 0.5
                        local color = Index == 1 and NewRGB(0, 0, 0) or NewRGB(255, 255, 255)
                        --
                        local preview_skeleton_head = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["Head"][2].Size.X * 0.5) + skeletonoffset.X, (window.VisualPreview.Components.Chams["Head"][2].Size.Y * 0.5) + skeletonoffset.Y), window.VisualPreview.Components.Chams["Head"][2]}, {
                        ...
                        --
                        local preview_skeleton_torso = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["Torso"][2].Size.X * 0) + skeletonoffset.X - (window.VisualPreview.Components.Chams["LeftArm"][2].Size.X / 2) + (Index == 1 and 3 or 1), skeletonoffset.Y), window.VisualPreview.Components.Chams["Torso"][2]}, {
                        ...
                        --
                        local preview_skeleton_leftarm = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["LeftArm"][2].Size.X * 0.5) + skeletonoffset.X, skeletonoffset.Y), window.VisualPreview.Components.Chams["LeftArm"][2]}, {
                        ...
                        --
                        local preview_skeleton_rightarm = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["RightArm"][2].Size.X * 0.5) + skeletonoffset.X, skeletonoffset.Y), window.VisualPreview.Components.Chams["RightArm"][2]}, {
                        ...
                        --
                        local preview_skeleton_hips = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["LeftLeg"][2].Size.X * 1) + skeletonoffset.X - (window.VisualPreview.Components.Chams["LeftLeg"][2].Size.X / 2) + (Index == 1 and 3 or 1), skeletonoffset.Y), window.VisualPreview.Components.Chams["LeftLeg"][2]}, {
                        ...
                        --
                        local preview_skeleton_leftleg = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["LeftLeg"][2].Size.X * 0.5) + skeletonoffset.X, skeletonoffset.Y), window.VisualPreview.Components.Chams["LeftLeg"][2]}, {
                        ...
                        --
                        local preview_skeleton_rightleg = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["RightLeg"][2].Size.X * 0.5) + skeletonoffset.X, skeletonoffset.Y), window.VisualPreview.Components.Chams["RightLeg"][2]}, {
                        ...
                        --
                        local preview_skeleton_hipstorso = utility:Create("Frame", {NewVector2((window.VisualPreview.Components.Chams["Torso"][2].Size.X * 0.5) + skeletonoffset.X, skeletonoffset.Y + (Index == 1 and 3 or 1)), window.VisualPreview.Components.Chams["Torso"][2]}, {
                ...
                --
                local preview_boxfill = utility:Create("Frame", {NewVector2(1, 1), preview_boxoutline}, {
                ...
                --
                local preview_flags = utility:Create("TextLabel", {NewVector2(preview_box.Size.X -56, 5), preview_box}, {
                    Text = "Flags ->", --Display\nMoving\nJumping\nDesynced"
                    Size = theme.textsize,
                    Font = theme.font,
                    Color = NewRGB(255, 255, 255),
                ...
                --
                local preview_healthbarvalue = utility:Create("TextLabel", {NewVector2(0, 5), preview_heatlhbar}, {
                    Text = "<- Number", --Display\nMoving\nJumping\nDesynced"
                    Size = theme.textsize,
                    Font = theme.font,
            ...
            do -- New Drawings
                ...
                for Index, Value in pairs(library.drawings) do
                    if Value[1] and Find(window.VisualPreview.Drawings, Value[1]) then
        ...
        function window:Watermark(info)
            ...
            --
            local watermark_outline = utility:Create("Frame", {NewVector2(100,38/2-10)}, {
            ...
            --
            local watermark_inline = utility:Create("Frame", {NewVector2(1,1), watermark_outline}, {
            ...
            --
            local watermark_frame = utility:Create("Frame", {NewVector2(1,1), watermark_inline}, {
            ...
            --
            local watermark_accent = utility:Create("Frame", {NewVector2(0,0), watermark_frame}, {
            ...
            --
            local watermark_title = utility:Create("TextLabel", {NewVector2(2 + 6,4), watermark_outline}, {
            ...
            Spawn(function()
                local count = 0
                utility:Connection(RenderStepped, function()
                    count = count + 1
                end)
            ...
            --
            utility:Connection(RenderStepped, function(FPS)
                library.shared.ping = Stats.Network:FindFirstChild("ServerStatsItem") and tostring(Floor(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())) or "Unknown"
                --
                if (tick() - Tick) > 0.15 then
                    watermark_title.Text = string.format("$anity | Ping : %s | Fps : %u", library.shared.ping, library.shared.fps)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
        ...
        function window:NotificationList(info)
            ...
            function window.notificationlist:AddNotification(info)
                local info = info or {}
                local text = info.text or info.Text or "$anity | Example Notification"
                local lifetime = info.lifetime or 5
                --
                local notification = {pressed = false}
                --
                
                local notify_outline = utility:Create("Frame", {NewVector2(0, 0)}, {
                ...
                --
                local notify_inline = utility:Create("Frame", {NewVector2(1, 1), notify_outline}, {
                ...
                --
                local notify_frame = utility:Create("Frame", {NewVector2(1, 1), notify_inline}, {
                ...
                --
                local notify_accent = utility:Create("Frame", {NewVector2(0, 0), notify_frame}, {
                ...
                --
                local notify_title = utility:Create("TextLabel", {NewVector2(8, 5), notify_frame}, {
                    Size = 13,
                ...
                --
                local notify__gradient = utility:Create("Image", {NewVector2(0, 0), notify_frame}, {
        ...
        function window:KeybindsList(info)
            ...
            local info = info or {}
            --
            local keybindslist_outline = utility:Create("Frame", {NewVector2(10,(utility:GetScreenSize().Y/2)-200)}, {
            ...
            --
            local keybindslist_inline = utility:Create("Frame", {NewVector2(1,1), keybindslist_outline}, {
            ...
            --
            local keybindslist_frame = utility:Create("Frame", {NewVector2(1,1), keybindslist_inline}, {
            ...
            --
            local keybindslist_accent = utility:Create("Frame", {NewVector2(0,0), keybindslist_frame}, {
            ...
            --
            local keybindslist_title = utility:Create("TextLabel", {NewVector2(keybindslist_outline.Size.X/2,4), keybindslist_outline}, {
                Text = "Keybinds",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            function window.keybindslist:Add(keybindname, keybindvalue)
                if keybindname and keybindvalue and not window.keybindslist.keybinds[keybindname] then
                    local keybindTable = {}
                    --
                    local keybind_outline = utility:Create("Frame", {NewVector2(0,keybindslist_outline.Size.Y-1), keybindslist_outline}, {
                    ...
                    --
                    local keybind_inline = utility:Create("Frame", {NewVector2(1,1), keybind_outline}, {
                    ...
                    --
                    local keybind_frame = utility:Create("Frame", {NewVector2(1,1), keybind_inline}, {
                    ...
                    --
                    local keybind_title = utility:Create("TextLabel", {NewVector2(4,3), keybind_outline}, {
                        Text = keybindname,
                        Size = theme.textsize,
                        Font = theme.font,
                        Color = theme.textcolor,
                    ...
                    --
                    local keybind_value = utility:Create("TextLabel", {NewVector2(keybind_outline.Size.X - 4 - utility:GetTextBounds(keybindname, theme.textsize, theme.font).X,3), keybind_outline}, {
                        Text = "["..keybindvalue.."]",
                        Size = theme.textsize,
                        Font = theme.font,
                        Color = theme.textcolor,
            ...
            --
            utility:Connection(Camera:GetPropertyChangedSignal("ViewportSize"),function()
        ...
        function window:StatusList(info)
            ...
            local info = info or {}
            --
            local statuslist_outline = utility:Create("Frame", {NewVector2(10,(utility:GetScreenSize().Y/2)-200)}, {
            ...
            --
            local statuslist_inline = utility:Create("Frame", {NewVector2(1,1), statuslist_outline}, {
            ...
            --
            local statuslist_frame = utility:Create("Frame", {NewVector2(1,1), statuslist_inline}, {
            ...
            --
            local statuslist_accent = utility:Create("Frame", {NewVector2(0,0), statuslist_frame}, {
            ...
            --
            local statuslist_title = utility:Create("TextLabel", {NewVector2(statuslist_outline.Size.X/2,4), statuslist_outline}, {
                Text = "Status",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            function window.statuslist:Add(statusname)
                if statusname and not window.statuslist.statuses[statusname] then
                    local statusTable = {}
                    --
                    local status_outline = utility:Create("Frame", {NewVector2(0,statuslist_outline.Size.Y-1), statuslist_outline}, {
                    ...
                    --
                    local status_inline = utility:Create("Frame", {NewVector2(1,1), status_outline}, {
                    ...
                    --
                    local status_frame = utility:Create("Frame", {NewVector2(1,1), status_inline}, {
                    ...
                    --
                    local status_title = utility:Create("TextLabel", {NewVector2(4,3), status_outline}, {
                        Text = statusname,
                        Size = theme.textsize,
                        Font = theme.font,
                        Color = theme.textcolor,
        ...
        function window:Cursor(info)
            ...
            utility:Connection(RenderStepped, function()
                ...
                local mouseLocation = utility:MouseLocation()
                --
                cursor.PointA = NewVector2(mouseLocation.X, mouseLocation.Y)
                cursor.PointB = NewVector2(mouseLocation.X + 12, mouseLocation.Y + 4)
                cursor.PointC = NewVector2(mouseLocation.X + 4, mouseLocation.Y + 12)
                --
                cursor_inline.PointA = NewVector2(mouseLocation.X, mouseLocation.Y)
                cursor_inline.PointB = NewVector2(mouseLocation.X + 12, mouseLocation.Y + 4)
                cursor_inline.PointC = NewVector2(mouseLocation.X + 4, mouseLocation.Y + 12)
            end)
            --
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and window.isVisible and utility:MouseOverDrawing({main_frame.Position.X,main_frame.Position.Y,main_frame.Position.X + main_frame.Size.X,main_frame.Position.Y + 20}) then
                ...
                window.drag = NewVector2(mouseLocation.X - main_frame.Position.X, mouseLocation.Y - main_frame.Position.Y)
            end
        end
        --
        ...
        library.changed[#library.changed + 1] = function(Input)
            if window.dragging and window.isVisible then
                ...
                if utility:GetScreenSize().Y-main_frame.Size.Y-5 > 5 then
                    local move = NewVector2(Clamp(mouseLocation.X - window.drag.X, 5, utility:GetScreenSize().X-main_frame.Size.X-5), Clamp(mouseLocation.Y - window.drag.Y, 5, utility:GetScreenSize().Y-main_frame.Size.Y-5))
                    window:Move(move)
                else
                    local move = NewVector2(mouseLocation.X - window.drag.X, mouseLocation.Y - window.drag.Y)
                    window:Move(move)
                end
            end
        end
        ...
        --
        utility:Connection(UserInputService.InputChanged,function()
            for _, func in pairs(library.changed) do
                local e,s = pcall(function()
                    func()
                end)
        ...
        --
        utility:Connection(Camera:GetPropertyChangedSignal("ViewportSize"),function()
            window:Move(NewVector2((utility:GetScreenSize().X/2) - (size.X/2), (utility:GetScreenSize().Y/2) - (size.Y/2)))
        end)
        --
    ...
    function library:Page(info)
        ...
        --
        local page_button = utility:Create("Frame", {NewVector2(position,4), window.back_frame}, {
        ...
        --
        local page_button_inline = utility:Create("Frame", {NewVector2(1,1), page_button}, {
        ...
        --
        local page_button_color = utility:Create("Frame", {NewVector2(1,1), page_button_inline}, {
        ...
        --
        local page_button_title = utility:Create("TextLabel", {NewVector2(utility:Position(0.5, 0, 0, 2, page_button_color).X - page_button_color.Position.X,2), page_button_color}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        function page:Update()
            ...
            for i,v in pairs(page.sections) do
                if v.side then
                    utility:UpdateOffset(v.section_inline, {NewVector2(v.side == "right" and (window.tab_frame.Size.X/2)+2 or 5,5 + page["sectionOffset"][v.side]), window.tab_frame})
    ...
    function pages:Section(info)
        ...
        local section_inline = utility:Create("Frame", {NewVector2(side == "right" and (window.tab_frame.Size.X/2)+2 or 5,5 + page["sectionOffset"][side]), window.tab_frame}, {
        ...
        --
        local section_outline = utility:Create("Frame", {NewVector2(1,1), section_inline}, {
        ...
        --
        local section_frame = utility:Create("Frame", {NewVector2(1,1), section_outline}, {
        ...
        --
        local section_accent = utility:Create("Frame", {NewVector2(0,0), section_frame}, {
        ...
        --
        local section_title = utility:Create("TextLabel", {NewVector2(3,3), section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
    ...
    function pages:MultiSection(info)
        ...
        --
        local multiSection_inline = utility:Create("Frame", {NewVector2(side == "right" and (window.tab_frame.Size.X/2)+2 or 5,5 + page["sectionOffset"][side]), window.tab_frame}, {
        ...
        --
        local multiSection_outline = utility:Create("Frame", {NewVector2(1,1), multiSection_inline}, {
        ...
        --
        local multiSection_frame = utility:Create("Frame", {NewVector2(1,1), multiSection_outline}, {
        ...
        --
        local multiSection_backFrame = utility:Create("Frame", {NewVector2(0,2), multiSection_frame}, {
        ...
        --
        local multiSection_bottomFrame = utility:Create("Frame", {NewVector2(0,multiSection_backFrame.Size.Y - 1), multiSection_backFrame}, {
        ...
        --
        local multiSection_accent = utility:Create("Frame", {NewVector2(0,0), multiSection_frame}, {
        ...
        for i,v in pairs(msections) do
            ...
            --
            local msection_frame = utility:Create("Frame", {NewVector2(multiSection.xAxis,0), multiSection_backFrame}, {
            ...
            --
            local msection_line = utility:Create("Frame", {NewVector2(msection_frame.Size.X,0), msection_frame}, {
            ...
            --
            local msection_title = utility:Create("TextLabel", {NewVector2(msection_frame.Size.X * 0.5,1), msection_frame}, {
                Text = v,
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            --
            local msection_bottomline = utility:Create("Frame", {NewVector2(0,msection_frame.Size.Y), msection_frame}, {
    ...
    function pages:PlayerList(info)
        ...
        --
        local playerList_inline = utility:Create("Frame", {NewVector2(5,5), window.tab_frame}, {
        ...
        --
        local playerList_outline = utility:Create("Frame", {NewVector2(1,1), playerList_inline}, {
        ...
        --
        local playerList_frame = utility:Create("Frame", {NewVector2(1,1), playerList_outline}, {
        ...
        --
        local playerList_accent = utility:Create("Frame", {NewVector2(0,0), playerList_frame}, {
        ...
        --
        local playerList_title = utility:Create("TextLabel", {NewVector2(3,3), playerList_frame}, {
        ...
        --
        local list_outline = utility:Create("Frame", {NewVector2(4,20), playerList_frame}, {
        ...
        --
        local list_inline = utility:Create("Frame", {NewVector2(1,1), list_outline}, {
        ...
        --
        local list_frame = utility:Create("Frame", {NewVector2(1,1), list_inline}, {
        ...
        --
        local list_scroll = utility:Create("Frame", {NewVector2(list_inline.Size.X - 9,1), list_inline}, {
        ...
        --
        local list_bar = utility:Create("Frame", {NewVector2(1,1), list_scroll}, {
        ...
        --
        local list_gradient = utility:Create("Image", {NewVector2(0,0), list_frame}, {
        ...
        for Index = 1, 10 do
            ...
            --
            if Index ~= 10 then
                listitem_line = utility:Create("Frame", {NewVector2(3,listitemposition + 21), list_frame}, {
            ...
            --
            local listitem_firstline = utility:Create("Frame", {NewVector2(1/3 * list_frame.Size.X,listitemposition + 3), list_frame}, {
            ...
            --
            local listitem_secondline = utility:Create("Frame", {NewVector2(2/3 * list_frame.Size.X,listitemposition + 3), list_frame}, {
                Size = utility:Size(0, 2, 0, 16, list_frame),
            ...
            --
            local listitem_username = utility:Create("TextLabel", {NewVector2(4, 4 + listitemposition), list_frame}, {
                Text = "",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            --
            local listitem_team = utility:Create("TextLabel", {NewVector2(6 + (1/3 * list_frame.Size.X), 4 + listitemposition), list_frame}, {
                Text = "",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            --
            local listitem_status = utility:Create("TextLabel", {NewVector2(6 + (2/3 * list_frame.Size.X), 4 + listitemposition), list_frame}, {
                Text = "",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            function item:Set(enabled, selected)
                ...
                if enabled then
                    ...
                    listitem_team.Color = selected[1].TeamColor.Color or NewRGB(255,255,255)
                    listitem_status.Text = selected[3]
                    --
                    listitem_username.Color = selected[4] and theme.accent or theme.textcolor
                    listitem_status.Color = selected[3] == "Local Player" and NewRGB(200, 55, 200) or selected[3] == "Priority" and NewRGB(51,158,206) or selected[3] == "Friend" and NewRGB(55, 200, 55) or selected[3] == "Enemy" and NewRGB(51,158,206) or theme.textcolor
                    --
                    library.colors[listitem_username] = {
                        OutlineColor = "textborder",
        ...
        --
        local options_iconoutline = utility:Create("Frame", {NewVector2(0,list_outline.Size.Y + 4), list_outline}, {
        ...
        --
        local options_iconinline = utility:Create("Frame", {NewVector2(1,1), options_iconoutline}, {
        ...
        --
        local options_iconframe = utility:Create("Frame", {NewVector2(1,1), options_iconinline}, {
        ...
        --
        local options_avatar = utility:Create("Image", {NewVector2(0,0), options_iconframe}, {
        ...
        --
        local options_loadingtext = utility:Create("TextLabel", {NewVector2((options_iconoutline.Size.X / 2) - 1, (options_iconoutline.Size.X / 2) - 10), options_iconframe}, {
            Text = "..?",
            Size = theme.textsize,
            Font = theme.font,
        ...
        --
        local options_title = utility:Create("TextLabel", {NewVector2(options_iconoutline.Size.X + 5, 0), options_iconoutline}, {
        ...
        for Index = 1, 1 do
            ...
            --
            local button_outline = utility:Create("Frame", {NewVector2(list_outline.Size.X - 180, list_outline.Size.Y + (Index == 1 and 10 or 36)), list_outline}, {
            ...
            --
            local button_inline = utility:Create("Frame", {NewVector2(1,1), button_outline}, {
            ...
            --
            local button_frame = utility:Create("Frame", {NewVector2(1,1), button_inline}, {
            ...
            --
            local button_gradient = utility:Create("Image", {NewVector2(0,0), button_frame}, {
            ...
            --
            local button_title = utility:Create("TextLabel", {NewVector2(button_frame.Size.X/2,1), button_frame}, {
            ...
            --
            local button_image = utility:Create("Image", {NewVector2(button_frame.Size.X - 15,button_frame.Size.Y/2 - 3), button_frame}, {
            ...
            function button:UpdateValue()
                if button.open and button.holder.inline then
                    for i,v in pairs(button.holder.buttons) do
                        ...
                        utility:UpdateOffset(v[1], {NewVector2(v[1].Text == tostring(button.current) and 8 or 6, 2), v[2]})
                    end
                end
            end
            --
            ...
            function button:Open()
                ...
                --
                local button_open_outline = utility:Create("Frame", {NewVector2(0,21), button_outline}, {
                ...
                --
                local button_open_inline = utility:Create("Frame", {NewVector2(1,1), button_open_outline}, {
                ...
                for Index = 1, (#button.options) do
                    ...
                    --
                    if Value then
                        local button_value_frame = utility:Create("Frame", {NewVector2(1,1 + (19 * (Index-1))), button_open_inline}, {
                        ...
                        --
                        local button_value = utility:Create("TextLabel", {NewVector2(Value == tostring(button.current) and 8 or 6,2), button_value_frame}, {
                            Text = Value,
                            Size = theme.textsize,
                            Font = theme.font,
        ...
        function playerList:UpdateScroll()
            if (#playerList.players - 10) > 0 then
                ...
                utility:UpdateOffset(list_bar, {NewVector2(1, 1 + ((((list_scroll.Size.Y - 2) - list_bar.Size.Y) / (#playerList.players - 10)) * playerList.scrollingindex)), list_scroll})
            else
        ...
        function playerList:Refresh(Relation)
            ...
            if Selection then
                if lastselection ~= Selection then
                    ...
                    --
                    local pImageData = HttpService:JSONDecode(game:HttpGet(("https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=%s&size=352x352&format=Png&isCircular=false"):format(Selection[1].UserId)))
    ...
    function sections:Label(info)
        ...
        local color = info.textcolor or NewRGB(255, 255, 255)
        --
        local window = self.window
        ...
        --
        local label_title = utility:Create("TextLabel", {NewVector2(middle and (section.section_frame.Size.X/2) or 4,label.axis), section.section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
    ...
    function sections:Textbox(info)
        ...
        --
        local textbox_outline = utility:Create("Frame", {NewVector2(4, textbox.axis), section.section_frame}, {
        ...
        --
        local textbox_inline = utility:Create("Frame", {NewVector2(1, 1), textbox_outline}, {
        ...
        --
        local textbox_frame = utility:Create("Frame", {NewVector2(1, 1), textbox_inline}, {
        ...
        --
        local placeholder_text = utility:Create("TextLabel", {NewVector2(middle and textbox_frame.Size.X / 2 or 4, 0), textbox_frame}, {
            Text = placeholder,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        --
        local text = utility:Create("TextLabel", {NewVector2(middle and textbox_frame.Size.X / 2 or 4, 0), textbox_frame}, {
            Text = text,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
    ...
    function sections:Toggle(info)
        ...
        --
        local toggle_outline = utility:Create("Frame", {NewVector2(4,toggle.axis), section.section_frame}, {
        ...
        --
        local toggle_inline = utility:Create("Frame", {NewVector2(1,1), toggle_outline}, {
        ...
        --
        local toggle_frame = utility:Create("Frame", {NewVector2(1,1), toggle_inline}, {
        ...
        --
        local toggle__gradient = utility:Create("Image", {NewVector2(0,0), toggle_frame}, {
        ...
        --
        local toggle_title = utility:Create("TextLabel", {NewVector2(23,toggle.axis + (15/2) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        function toggle:Colorpicker(info)
            ...
            local def = info.def or info.Def or info.default or info.Default or NewRGB(255, 255,255)
            ...
            --
            local colorpicker_outline = utility:Create("Frame", {NewVector2(section.section_frame.Size.X-(toggle.colorpickers == 0 and (30+4) or (64 + 4)),colorpicker.axis), section.section_frame}, {
            ...
            --
            local colorpicker_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_outline}, {
            ...
            if transp then
                colorpicker__transparency = utility:Create("Image", {NewVector2(1,1), colorpicker_inline}, {
            ...
            --
            local colorpicker_frame = utility:Create("Frame", {NewVector2(1,1), colorpicker_inline}, {
                Size = utility:Size(1, -2, 1, -2, colorpicker_inline),
                Position = utility:Position(0, 1, 0, 1, colorpicker_inline),
                Color = def,
            ...
            --
            local colorpicker__gradient = utility:Create("Image", {NewVector2(0,0), colorpicker_frame}, {
            ...
            function colorpicker:Refresh()
                ...
                if colorpicker.open and colorpicker.holder.picker and colorpicker.holding.picker then
                    ...
                    --
                    utility:UpdateOffset(colorpicker.holder.picker_cursor, {NewVector2((colorpicker.holder.picker.Size.X*colorpicker.current[2])-3,(colorpicker.holder.picker.Size.Y*(1-colorpicker.current[3]))-3), colorpicker.holder.picker})
                    --
                ...
                elseif colorpicker.open and colorpicker.holder.huepicker and colorpicker.holding.huepicker then
                    ...
                    --
                    utility:UpdateOffset(colorpicker.holder.huepicker_cursor[1], {NewVector2(-3,(colorpicker.holder.huepicker.Size.Y*colorpicker.current[1])-3), colorpicker.holder.huepicker})
                    --
                ...
                elseif colorpicker.open and colorpicker.holder.transparency and colorpicker.holding.transparency then
                    ...
                    utility:UpdateOffset(colorpicker.holder.transparency_cursor[1], {NewVector2((colorpicker.holder.transparency.Size.X*(1-colorpicker.current[4]))-3,-3), colorpicker.holder.transparency})
                    --
                    colorpicker.holder.background.Color = Color3.fromHSV(colorpicker.current[1], 1, 1)
                end
                --
                ...
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and colorpicker_outline.Visible then
                    ...
                    elseif utility:MouseOverDrawing({section.section_frame.Position.X + (section.section_frame.Size.X - (colorpicker.index == 0 and (30 + 4 + 2) or (64 + 4 + 2))), section.section_frame.Position.Y + colorpicker.axis, section.section_frame.Position.X + section.section_frame.Size.X - (colorpicker.index == 1 and 36 or 0), section.section_frame.Position.Y + colorpicker.axis + 15}) and not window:IsOverContent() then
                        if not colorpicker.open then
                            ...
                            colorpicker.open = not colorpicker.open
                            --
                            local colorpicker_open_outline = utility:Create("Frame", {NewVector2(4,colorpicker.axis + 19), section.section_frame}, {
                            ...
                            --
                            local colorpicker_open_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_outline}, {
                            ...
                            --
                            local colorpicker_open_frame = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_inline}, {
                            ...
                            --
                            local colorpicker_open_accent = utility:Create("Frame", {NewVector2(0,0), colorpicker_open_frame}, {
                            ...
                            --
                            local colorpicker_title = utility:Create("TextLabel", {NewVector2(4,2), colorpicker_open_frame}, {
                                Text = cpinfo,
                                Size = theme.textsize,
                                Font = theme.font,
                                Color = theme.textcolor,
                            ...
                            --
                            local colorpicker_open_picker_outline = utility:Create("Frame", {NewVector2(4,17), colorpicker_open_frame}, {
                            ...
                            --
                            local colorpicker_open_picker_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_picker_outline}, {
                            ...
                            --
                            local colorpicker_open_picker_bg = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_picker_inline}, {
                            ...
                            --
                            local colorpicker_open_picker_image = utility:Create("Image", {NewVector2(0,0), colorpicker_open_picker_bg}, {
                            ...
                            --
                            local colorpicker_open_picker_cursor = utility:Create("Image", {NewVector2((colorpicker_open_picker_image.Size.X*colorpicker.current[2])-3,(colorpicker_open_picker_image.Size.Y*(1-colorpicker.current[3]))-3), colorpicker_open_picker_image}, {
                            ...
                            --
                            local colorpicker_open_huepicker_outline = utility:Create("Frame", {NewVector2(colorpicker_open_frame.Size.X-19,17), colorpicker_open_frame}, {
                            ...
                            --
                            local colorpicker_open_huepicker_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_huepicker_outline}, {
                            ...
                            --
                            local colorpicker_open_huepicker_image = utility:Create("Image", {NewVector2(1,1), colorpicker_open_huepicker_inline}, {
                            ...
                            --
                            local colorpicker_open_huepicker_cursor_outline = utility:Create("Frame", {NewVector2(-3,(colorpicker_open_huepicker_image.Size.Y*colorpicker.current[1])-3), colorpicker_open_huepicker_image}, {
                            ...
                            --
                            local colorpicker_open_huepicker_cursor_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_huepicker_cursor_outline}, {
                            ...
                            --
                            local colorpicker_open_huepicker_cursor_color = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_huepicker_cursor_inline}, {
                            ...
                            --
                            if transp then
                                local colorpicker_open_transparency_outline = utility:Create("Frame", {NewVector2(4,colorpicker_open_frame.Size.X-19), colorpicker_open_frame}, {
                                ...
                                --
                                local colorpicker_open_transparency_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_transparency_outline}, {
                                ...
                                --
                                local colorpicker_open_transparency_bg = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_transparency_inline}, {
                                ...
                                --
                                local colorpicker_open_transparency_image = utility:Create("Image", {NewVector2(1,1), colorpicker_open_transparency_inline}, {
                                    Size = utility:Size(1, -2, 1, -2, colorpicker_open_transparency_inline),
                                    Position = utility:Position(0, 1, 0 , 1, colorpicker_open_transparency_inline),
                                }, colorpicker.holder.drawings);colorpicker.holder.transparency = colorpicker_open_transparency_image
                                --
                                local colorpicker_open_transparency_cursor_outline = utility:Create("Frame", {NewVector2((colorpicker_open_transparency_image.Size.X*(1-colorpicker.current[4]))-3,-3), colorpicker_open_transparency_image}, {
                                ...
                                --
                                local colorpicker_open_transparency_cursor_inline = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_transparency_cursor_outline}, {
                                ...
                                --
                                local colorpicker_open_transparency_cursor_color = utility:Create("Frame", {NewVector2(1,1), colorpicker_open_transparency_cursor_inline}, {
        ...
        function toggle:Keybind(info)
            ...
            --
            local keybind_outline = utility:Create("Frame", {NewVector2(section.section_frame.Size.X-(40+4),keybind.axis), section.section_frame}, {
            ...
            --
            local keybind__gradient = utility:Create("Image", {NewVector2(0,0), keybind_frame}, {
            ...
            --
            local keybind_value = utility:Create("TextLabel", {NewVector2(keybind_outline.Size.X/2,1), keybind_outline}, {
                Text = "...",
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
            ...
            library.began[#library.began + 1] = function(Input)
                ...
                if Input.UserInputType == Enum.UserInputType.MouseButton2 and window.isVisible and keybind_outline.Visible then
                    if utility:MouseOverDrawing({section.section_frame.Position.X  + (section.section_frame.Size.X - (40+4+2)), section.section_frame.Position.Y + keybind.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + keybind.axis + 17}) and not window:IsOverContent() and not keybind.selecting then
                        ...
                        keybind.open = not keybind.open
                        --
                        local modemenu = utility:Create("Frame", {NewVector2(keybind_outline.Size.X + 2,0), keybind_outline}, {
                        ...
                        --
                        local modemenu_inline = utility:Create("Frame", {NewVector2(1,1), modemenu}, {
                        ...
                        --
                        local modemenu_frame = utility:Create("Frame", {NewVector2(1,1), modemenu_inline}, {
                        ...
                        --
                        local keybind__gradient = utility:Create("Image", {NewVector2(0,0), modemenu_frame}, {
                        ...
                        for i,v in pairs({"Always", "Toggle", "Hold"}) do
                            local button_title = utility:Create("TextLabel", {NewVector2(modemenu_frame.Size.X/2,15 * (i-1)), modemenu_frame}, {
                                Text = v,
                                Size = theme.textsize,
                                Font = theme.font,
    ...
    function sections:Slider(info)
        ...
        --
        if name then
            local slider_title = utility:Create("TextLabel", {NewVector2(4,slider.axis), section.section_frame}, {
                Text = name,
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
        ...
        --
        local slider_outline = utility:Create("Frame", {NewVector2(4,slider.axis + (name and 15 or 0)), section.section_frame}, {
        ...
        --
        local slider_inline = utility:Create("Frame", {NewVector2(1,1), slider_outline}, {
        ...
        --
        local slider_frame = utility:Create("Frame", {NewVector2(1,1), slider_inline}, {
        ...
        --
        local slider_slide = utility:Create("Frame", {NewVector2(1,1), slider_inline}, {
        ...
        --
        local slider__gradient = utility:Create("Image", {NewVector2(0,0), slider_frame}, {
        ...
        --
        local textBounds = utility:GetTextBounds(name, theme.textsize, theme.font)
        local slider_value = utility:Create("TextLabel", {NewVector2(slider_outline.Size.X/2,(slider_outline.Size.Y/2) - (textBounds.Y/2)), slider_outline}, {
        ...
        function slider:Refresh()
            ...
            local value = Floor((slider.min + (slider.max - slider.min) * percent) * slider.decimals) / slider.decimals
    ...
    function sections:Button(info)
        ...
        --
        local button_outline = utility:Create("Frame", {NewVector2(4,button.axis), section.section_frame}, {
    ...
    function sections:ButtonHolder(info)
        ...
        for i=1, 2 do
            ...
            --
            local button_outline = utility:Create("Frame", {NewVector2(i == 2 and ((section.section_frame.Size.X / 2) + 2) or 4,button.axis), section.section_frame}, {
    ...
    function sections:Dropdown(info)
        ...
        --
        local dropdown_outline = utility:Create("Frame", {NewVector2(4,name and (dropdown.axis + 15) or dropdown.axis), section.section_frame}, {
            Size = utility:Size(1, -8, 0, 20, section.section_frame),
        ...
        --
        local dropdown_inline = utility:Create("Frame", {NewVector2(1,1), dropdown_outline}, {
        ...
        --
        local dropdown_frame = utility:Create("Frame", {NewVector2(1,1), dropdown_inline}, {
        ...
        --
        if name then
            local dropdown_title = utility:Create("TextLabel", {NewVector2(4,dropdown.axis), section.section_frame}, {
                Text = name,
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
        ...
        --
        --
        local dropdown_value = utility:Create("TextLabel", {NewVector2(3,dropdown_frame.Size.Y/2 - 7), dropdown_frame}, {
                ...
                if dropdown.scrollindex ~= lastupdate then
                    if max and dropdown.bar and dropdown.scroll then
                        ...
                        if (#dropdown.options - max) > 0 then
                            ...
                            utility:UpdateOffset(dropdown.bar, {NewVector2(1, (((dropdown.scroll.Size.Y - dropdown.bar.Size.Y) / (#dropdown.options - max)) * dropdown.scrollindex)), dropdown.scroll})
                        else
        ...
        function dropdown:Update()
            if dropdown.open and dropdown.holder.inline then
                for i,v in pairs(dropdown.holder.buttons) do
                    ...
                    utility:UpdateOffset(v[1], {NewVector2(v[1].Text == tostring(dropdown.current) and 8 or 6, 2), v[2]})
                end
            end
        end
        --
        function dropdown:set(value)
            if typeof(value) == "string" and Find(options, value) then
                dropdown.current = value
                dropdown_value.Text = value
                Spawn(callback, dropdown.current)
            end
        end
		function dropdown:add(value)
            Insert(options, tostring(value)) 
        end
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and dropdown_outline.Visible then
                ...
                elseif utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + dropdown.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + dropdown.axis + (name and (15 + 20) or (20))}) and not window:IsOverContent() then
                    ...
                    if not dropdown.open then
                        ...
                        dropdown.open = not dropdown.open
                        --
                        local dropdown_open_outline = utility:Create("Frame", {NewVector2(0,19), dropdown_outline}, {
                        ...
                        --
                        local dropdown_open_inline = utility:Create("Frame", {NewVector2(1,1), dropdown_open_outline}, {
                        ...
                        --
                        if max then
                            local dropdown_open_scroll = utility:Create("Frame", {NewVector2(dropdown_open_inline.Size.X - 5,1), dropdown_open_inline}, {
                            ...
                            --
                            local dropdown_open_bar = utility:Create("Frame", {NewVector2(0, (((dropdown_open_scroll.Size.Y - ((max / #dropdown.options) * dropdown_open_scroll.Size.Y)) / (#dropdown.options - max)) * dropdown.scrollindex)), dropdown_open_scroll}, {
                        ...
                        for Index = 1, (max and max or #dropdown.options) do
                            ...
                            --
                            if Value then
                                local dropdown_value_frame = utility:Create("Frame", {NewVector2(1,1 + (19 * (Index-1))), dropdown_open_inline}, {
                                ...
                                --
                                local dropdown_value = utility:Create("TextLabel", {NewVector2(Value == tostring(dropdown.current) and 8 or 6,2), dropdown_value_frame}, {
                                    Text = Value,
                                    Size = theme.textsize,
                                    Font = theme.font,
    ...
    function sections:Multibox(info)
        ...
        --
        local multibox_outline = utility:Create("Frame", {NewVector2(4,multibox.axis + 15), section.section_frame}, {
            Size = utility:Size(1, -8, 0, 20, section.section_frame),
        ...
        --
        local multibox_inline = utility:Create("Frame", {NewVector2(1,1), multibox_outline}, {
        ...
        --
        local multibox_frame = utility:Create("Frame", {NewVector2(1,1), multibox_inline}, {
        ...
        --
        local multibox_title = utility:Create("TextLabel", {NewVector2(4,multibox.axis), section.section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        --
        local multibox__gradient = utility:Create("Image", {NewVector2(0,0), multibox_frame}, {
        ...
        --
        local multibox_value = utility:Create("TextLabel", {NewVector2(3,multibox_frame.Size.Y/2 - 7), multibox_frame}, {
            Text = "",
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        --
        local multibox_image = utility:Create("Image", {NewVector2(multibox_frame.Size.X - 15,multibox_frame.Size.Y/2 - 3), multibox_frame}, {
        ...
        function multibox:Update()
            if multibox.open and multibox.holder.inline then
                for i,v in pairs(multibox.holder.buttons) do
                    ...
                    --
                    utility:UpdateOffset(v[1], {NewVector2(Find(multibox.current, v[1].Text) and 8 or 6, 2), v[2]})
                end
            end
        end
        --
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and multibox_outline.Visible then
                ...
                elseif utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + multibox.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + multibox.axis + 15 +  20}) and not window:IsOverContent() then
                    if not multibox.open then
                        ...
                        --
                        local multibox_open_outline = utility:Create("Frame", {NewVector2(0,19), multibox_outline}, {
                        ...
                        --
                        local multibox_open_inline = utility:Create("Frame", {NewVector2(1,1), multibox_open_outline}, {
                        ...
                        for i,v in pairs(options) do
                            local multibox_value_frame = utility:Create("Frame", {NewVector2(1,1 + (19 * (i-1))), multibox_open_inline}, {
                            ...
                            --[[
                            local multibox_value_gradient = utility:Create("Image", {NewVector2(0,0), multibox_value_frame}, {
                            ...
                            --
                            local multibox_value = utility:Create("TextLabel", {NewVector2(Find(multibox.current, v) and 8 or 6,2), multibox_value_frame}, {
                                Text = v,
                                Size = theme.textsize,
                                Font = theme.font,
    ...
    function sections:Keybind(info)
        ...
        --
        local keybind_title = utility:Create("TextLabel", {NewVector2(4,keybind.axis + (17/2) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
    ...
    function sections:Colorpicker(info)
        ...
        local cpinfo = info.info or info.Info or name
        local def = info.def or info.Def or info.default or info.Default or NewRGB(255, 0, 0)
        local transp = info.transparency or info.Transparency or info.transp or info.Transp or info.alpha or info.Alpha or nil
        local pointer = info.pointer or info.Pointer or info.flag or info.Flag or nil
        local callback = info.callback or info.callBack or info.Callback or info.CallBack or function()end
        --
        ...
        --
        local colorpicker_outline = utility:Create("Frame", {NewVector2(section.section_frame.Size.X-(30+4),colorpicker.axis), section.section_frame}, {
            Size = utility:Size(0, 30, 0, 15),
        ...
        --
        local colorpicker_title = utility:Create("TextLabel", {NewVector2(4,colorpicker.axis + (15/2) - (utility:GetTextBounds(name, theme.textsize, theme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
        ...
        function colorpicker:Colorpicker(info)
            ...
            utility:UpdateOffset(colorpicker_outline, {NewVector2(section.section_frame.Size.X-(60+8),colorpicker.axis), section.section_frame})
            --
            local colorpicker_outline = utility:Create("Frame", {NewVector2(section.section_frame.Size.X-(30+4),colorpicker.axis), section.section_frame}, {
                Size = utility:Size(0, 30, 0, 15),
                Position = utility:Position(1, -(30+4), 0, colorpicker.axis, section.section_frame),
    ...
    function sections:Listbox(info)
        ...
        --
        local listbox_outline = utility:Create("Frame", {NewVector2(4,listbox.axis), section.section_frame}, {
        ...
        --
        local listbox_inline = utility:Create("Frame", {NewVector2(1,1), listbox_outline}, {
        ...
        --
        local listbox_frame = utility:Create("Frame", {NewVector2(1,1), listbox_inline}, {
        ...
        --
        local listbox_gradient = utility:Create("Image", {NewVector2(0,0), listbox_frame}, {
        ...
        --
        local listbox_scroll_outline = utility:Create("Frame", {NewVector2(listbox_gradient.Size.X - 4, 0), listbox_gradient}, {
        ...
        --
        local listbox_scroll_frame = utility:Create("Frame", {NewVector2(1, 1), listbox_scroll_outline}, {
        ...
        function listbox:RefreshScroll(down)
            for i = self.startindex, 1, -1 do
                ...
                utility:UpdateOffset(button, {NewVector2(listbox_frame.Size.X / 2, 2 + 18 * (#self.buttons - i)), listbox_frame})
                --
            ...
            for i = self.startindex, 7 + self.startindex do
                ...
                utility:UpdateOffset(button, {NewVector2(listbox_frame.Size.X / 2, 2 + 18 * (i - self.startindex)), listbox_frame})
                --
        ...
        function listbox:RefreshScrollBar()
            ...
            utility:UpdateOffset(listbox_scroll_frame, {NewVector2(1, 1 + scale * listbox_scroll_outline.Size.Y), listbox_scroll_outline})
            listbox_scroll_frame.Position = utility:Position(0, 1, scale, 1, listbox_scroll_outline)
            listbox_scroll_frame.Size = utility:Size(0, 2, Clamp(#self.buttons == 0 and 1 or 1 / (#self.buttons / 8), 0, 1), -2, listbox_scroll_outline)
        end
        --
        ...
        function listbox:UpdateList(list, dontset, keepscroll)
            ...
            for i, button in next, self.buttons do
                ...
                for i2, content in next, section.visibleContent do
                    if content == button then
                        Remove(section.visibleContent, i2)
                        break
                    end
                end
                --
            ...
            for i, name in ipairs(list) do
                local button_title = utility:Create("TextLabel", {NewVector2(listbox_frame.Size.X / 2, 2 + (18 * #listbox.buttons)), listbox_frame}, {
                    Text = name,
                    Size = theme.textsize,
                    Font = theme.font,
            ...
            if keepscroll then
                for index = 1, Clamp(self.startindex - 1, 1, #self.buttons - 8 <= 0 and 1 or #self.buttons - 8) do
            ...
            if not dontset then
                if multichoice then
                    ...
                    if list[1] then
                        self:set({{list[1], Find(list, list[1])}})
                    end
                end
            end
        end
    ...
    function sections:ConfigBox(info)
        ...
        --
        local configLoader_outline = utility:Create("Frame", {NewVector2(4,configLoader.axis), section.section_frame}, {
            Size = utility:Size(1, -8, 0, 148, section.section_frame),
        ...
        --
        local configLoader_inline = utility:Create("Frame", {NewVector2(1,1), configLoader_outline}, {
        ...
        --
        local configLoader_frame = utility:Create("Frame", {NewVector2(1,1), configLoader_inline}, {
        ...
        --
        local configLoader_gradient = utility:Create("Image", {NewVector2(0,0), configLoader_frame}, {
        ...
        --
        for i=1, 8 do
            local config_title = utility:Create("TextLabel", {NewVector2(configLoader_frame.Size.X/2,2 + (18 * (i-1))), configLoader_frame}, {
...
library.themes = {
    ["Interwebz"] = {
        ['Accent'] = NewRGB(172,89,66);
        ['lightcontrast'] = NewRGB(35,29,48);
        ['darkcontrast'] = NewRGB(27,19,37);
        ['outline'] = NewRGB(25,22,25);
        ['inline'] = NewRGB(53,45,64);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Wave"] = {
        ['Accent'] = NewRGB(67,147,176);
        ['lightcontrast'] = NewRGB(30,34,41);
        ['darkcontrast'] = NewRGB(26,30,35);
        ['outline'] = NewRGB(24,26,32);
        ['inline'] = NewRGB(39,48,53);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Default"] = {
        ['Accent'] = NewRGB(0,110,176);
        ['lightcontrast'] = NewRGB(31,30,31);
        ['darkcontrast'] = NewRGB(40,39,41);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(50, 50, 50);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Entropy"] = {
        ['Accent'] = NewRGB(147,188,222);
        ['lightcontrast'] = NewRGB(56,54,61);
        ['darkcontrast'] = NewRGB(40,39,46);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(63,63,67);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Dracula"] = {
        ['Accent'] = NewRGB(125,104,142);
        ['lightcontrast'] = NewRGB(35,37,48);
        ['darkcontrast'] = NewRGB(30,31,40);
        ['outline'] = NewRGB(29,30,39);
        ['inline'] = NewRGB(49,45,62);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Neverlose"] = {
        ['Accent'] = NewRGB(0,158,210);
        ['lightcontrast'] = NewRGB(0,13,23);
        ['darkcontrast'] = NewRGB(3,4,17);
        ['outline'] = NewRGB(1,1,1);
        ['inline'] = NewRGB(29,30,39);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Aimware"] = {
        ['Accent'] = NewRGB(175,34,34);
        ['lightcontrast'] = NewRGB(36,36,36);
        ['darkcontrast'] = NewRGB(22,22,22);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(43,43,44);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Spotify"] = {
        ['Accent'] = NewRGB(30,193,81);
        ['lightcontrast'] = NewRGB(19,19,19);
        ['darkcontrast'] = NewRGB(16,16,16);
        ['outline'] = NewRGB(0,0,0);
    ...
    ["Neko"] = {
        ['Accent'] = NewRGB(170,27,80);
        ['lightcontrast'] = NewRGB(19,19,18);
        ['darkcontrast'] = NewRGB(17,17,17);
        ['outline'] = NewRGB(25,22,25);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Gamesense"] = {
        ['Accent'] = NewRGB(105,188,18);
        ['lightcontrast'] = NewRGB(19,19,18);
        ['darkcontrast'] = NewRGB(17,17,17);
        ['outline'] = NewRGB(1,1,1);
        ['inline'] = NewRGB(39,39,38);
    ...
    ["Onetap"] = {
        ['Accent'] = NewRGB(188,141,78);
        ['lightcontrast'] = NewRGB(44,48,54);
        ['darkcontrast'] = NewRGB(31,33,35);
        ['outline'] = NewRGB(20,24,25);
        ['inline'] = NewRGB(50,49,49);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(44,48,54);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Vape"] = {
        ['Accent'] = NewRGB(1,190,136);
        ['lightcontrast'] = NewRGB(19,19,18);
        ['darkcontrast'] = NewRGB(17,17,17);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
    ...
    ["Corn"] = {
        ['Accent'] = NewRGB(249,151,4);
        ['lightcontrast'] = NewRGB(15,15,13);
        ['darkcontrast'] = NewRGB(17,17,17);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
    ...
    ["Minecraft"] = {
        ['Accent'] = NewRGB(105,153,44);
        ['lightcontrast'] = NewRGB(20,21,26);
        ['darkcontrast'] = NewRGB(10,11,16);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Fatality"] = {
        ['Accent'] = NewRGB(117,34,94);
        ['lightcontrast'] = NewRGB(31,24,65);
        ['darkcontrast'] = NewRGB(27,20,54);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Sublime"] = {
        ['Accent'] = NewRGB(254,146,12);
        ['lightcontrast'] = NewRGB(39,39,39);
        ['darkcontrast'] = NewRGB(30,30,30);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Youtube"] = {
        ['Accent'] = NewRGB(255,0,0);
        ['lightcontrast'] = NewRGB(29,29,29);
        ['darkcontrast'] = NewRGB(15,15,15);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Abyss"] = {
        ['Accent'] = NewRGB(91,99,183);
        ['lightcontrast'] = NewRGB(30,30,30);
        ['darkcontrast'] = NewRGB(26,26,26);
        ['outline'] = NewRGB(0,0,0);
        ['inline'] = NewRGB(39,39,38);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
    ...
    ["Nebula"] = {
        ['Accent'] = NewRGB(61, 100, 227);
        ['lightcontrast'] = NewRGB(41,41,50);
        ['darkcontrast'] = NewRGB(27,27,34);
        ['outline'] = NewRGB(1,1,1);
        ['inline'] = NewRGB(39,39,47);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Primordial"] = {
        ['Accent'] = NewRGB(188,156,165);
        ['lightcontrast'] = NewRGB(41,41,41);
        ['darkcontrast'] = NewRGB(31,31,31);
        ['outline'] = NewRGB(31,31,31);
        ['inline'] = NewRGB(21,21,21);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Seere"] = {
        ['Accent'] = NewRGB(35,131,167);
        ['lightcontrast'] = NewRGB(20,20,20);
        ['darkcontrast'] = NewRGB(17,17,17);
        ['outline'] = NewRGB(1,1,1);
        ['inline'] = NewRGB(28,28,28);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
    ["Azure"] = {
        ['Accent'] = NewRGB(222,117,222);
        ['lightcontrast'] = NewRGB(80,80,80);
        ['darkcontrast'] = NewRGB(65,65,65);
        ['outline'] = NewRGB(60,60,60);
        ['inline'] = NewRGB(72,72,72);
        ['textcolor'] = NewRGB(255,255,255);
        ['textborder'] = NewRGB(0,0,0);
        ['cursoroutline'] = NewRGB(10,10,10);
    };
...
function library:CreateSettings(setting_tab, windowarg)
    ...
    local function update_config_list()
        local list = {}
        for idx, file in ipairs(listfiles("Sanity/Presets")) do
            local file_name = file:gsub("Sanity/Presets\\", ""):gsub(".cfg", "")
    ...
    local_config:Button({
        ...
        callback = function()
            ...
            if config_name == "" or isfile("Sanity/Presets/" .. config_name .. ".cfg") then
                return
            end
            writefile("Sanity/Presets/" .. config_name .. ".cfg", windowarg:GetConfig())
            update_config_list()
            --
            --windowarg.notificationlist:AddNotification({Text = Format("$anity | Created config: %s!", config_name)})
    end})
    local_config:Button({
        name = "Load Preset",
        confirmation = true,
        callback = function()
            ...
            if selected_config then
                windowarg:LoadConfig(readfile("Sanity/Presets/" .. selected_config .. ".cfg"))
                --windowarg.notificationlist:AddNotification({Text = Format("$anity | Loaded config: %s!", config_name)})
            end
    end})
    local_config:Button({
        name = "Save Preset",
        ...
        callback = function()
            local selected_config = library.pointers["presetlist"]:Get()[1][1]
            if selected_config then
                writefile("Sanity/Presets/" .. selected_config .. ".cfg", windowarg:GetConfig())
                --windowarg.notificationlist:AddNotification({Text = Format("$anity | Saved config: %s!", config_name)})
            end
    end})
    local_config:Button({
        name = "Delete Preset",
        ...
        callback = function()
            local selected_config = library.pointers["presetlist"]:Get()[1][1]
            if selected_config then
                delfile("Sanity/Presets/" .. selected_config .. ".cfg")
                update_config_list()
                --windowarg.notificationlist:AddNotification({Text = Format("$anity | Deleted config: %s!", config_name)})
            end
    end})
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if AccentEffect == "Rainbow" and UserInputService.MouseIconEnabled == false then
            local Clock = os.clock() * EffectSpeed
            local Color = Color3.fromHSV(Abs(Sin(Clock)), 1, 1);
            library.pointers["accent"]:set(Color)
            library:UpdateColor("accent", Color);
        end;
        if AccentEffect == "Shift" and UserInputService.MouseIconEnabled == false then
            local ShiftOffset = 0
        ...
        if AccentEffect == "Reverse Shift" and UserInputService.MouseIconEnabled == false then
            local ShiftOffset = 0
            local Clock = os.clock() * EffectSpeed + ShiftOffset;
            ShiftOffset = ShiftOffset - 0.01;
            local Color = Color3.fromHSV(Abs(Sin(Clock)), 1, 1)
